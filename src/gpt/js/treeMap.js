...
function decorateFolioLabels(data, rows) {
  // Mapea folio -> origen (primera ocurrencia no vacía)
  ...
  // Pone 'f' (formatted label) sólo en el nivel de folio (parent === 'Tickets')
...
export function renderTreemap(
  ...
  // --- Reusar una instancia del TreeMap por container si quieres (opcional)
  ...
  // Limpia listeners viejos para no duplicar en cada draw
  ...
  google.visualization.events.addListener(tree, 'select', () => {
    ...
    const idPath = data.getValue(row, 0) || ''; // "Tickets│INC...│REGION│YYYY-MM-DD│..."
    ...
    // Dispara callback para que refresques draw(...) (Aviso + Modal)
  ...
  // Guardar redibujador para resize
